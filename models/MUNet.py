...
class MUNet(nn.Module):
    ...
    def forward(self, T1, flair, met_LR, lowRes):
        #print(met_code)
        x = F.interpolate(met_LR.unsqueeze(-1), size=T1.shape[2:], mode='trilinear', align_corners=True)

        enc1_1, enc1_2, enc1_3, enc1_4, bottleneck1 = self.encoder1(torch.cat((x, T1), dim=1))
        enc2_1, enc2_2, enc2_3, enc2_4, bottleneck2 = self.encoder2(torch.cat((x, flair), dim=1))
        ...
        output = data_consistency(met_LR, output, lowRes)
        return output
