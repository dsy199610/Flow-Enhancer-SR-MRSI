...
class cInvNet_MRI_ConditionalPrior(nn.Module):
    def __init__(self, channel_in=1, block_num=[12, 12, 12, 12], feature=128, down_num=3):
        super(cInvNet_MRI_ConditionalPrior, self).__init__()
    ...
    def prior(self, x, T1, flair, metname):
        MRI = F.interpolate(torch.cat((T1, flair), dim=1), size=(64, 64, 1), mode='trilinear', align_corners=True).squeeze(-1)
        c = torch.cat((x, MRI), dim=1)
        out = self.mapping(c)
        mean = out[:, :4096]
