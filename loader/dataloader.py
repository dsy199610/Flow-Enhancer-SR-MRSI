...
class twoD_Data(Dataset):
    def __init__(self, patients, transform, mode):
...
def RandomDownscale_function(met_HR, lowRes):
    ...
    div = lowRes
    kspace_trunc = kspace[:, :, 32 - div:32 + div, 32 - div:32 + div, :]
    met_LR_cplx = utils.ifft2(kspace_trunc)
    met_LR = torch.sqrt(met_LR_cplx[:, :, :, :, 0] ** 2 + met_LR_cplx[:, :, :, :, 1] ** 2)
    met_LR = met_LR * met_HR.mean(dim=(1, 2, 3), keepdim=True) / met_LR.mean(dim=(1, 2, 3), keepdim=True) # added
    return met_LR, lowRes


def RandomDownscale_function_v2(met_HR, lowRes):
    met_HR_cplx = torch.cat((met_HR.unsqueeze(-1), torch.zeros_like(met_HR.unsqueeze(-1))), -1)
    kspace = utils.fft2(met_HR_cplx)
    ...
    met_LR_cplx = utils.ifft2(kspace_trunc)
    met_LR = torch.sqrt(met_LR_cplx[:, :, :, :, 0] ** 2 + met_LR_cplx[:, :, :, :, 1] ** 2)# / ((32 / div)**2)
    return met_LR, lowRes


def RandomDownscale_function_random(met_HR, lowRes):
    met_HR_cplx = torch.cat((met_HR.unsqueeze(-1), torch.zeros_like(met_HR.unsqueeze(-1))), -1)
    kspace = utils.fft2(met_HR_cplx)
    kspace_trunc = torch.zeros_like(kspace)
